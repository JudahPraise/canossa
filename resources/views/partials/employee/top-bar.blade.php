  ...
  <ul class="navbar-nav ml-auto mt-2 mt-lg-0 d-flex flex-row">
    ...
    <div class="has-dropdown dropleft m-0 mt-2 px-2">
      ...
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="width: 25rem; height: 30rem; position: absolute; top: 2rem; left: -15rem">
        <div class="card-header" style="position: absolute; top: 0; height:10%; width: 100%; color: black; font-weight: bold; font-size: 1.2rem">Messages</div>
        <div class="card-body p-0 pt-2" style="position: absolute; top: 2.8rem; height:82%; width: 100%; overflow: scroll">
          <a class="dropdown-item d-flex flex-column announcement py-2" data-toggle="modal" data-target="#showAnnouncement">
            <h5 class="card-title">New messagae</h5>
    ...
    <div class="has-dropdown dropleft m-0 mt-2 px-2">
      ...
      @if(auth()->user()->unreadNotifications()->count())  
        <span class="badge badge-danger align-self-start">{{ auth()->user()->unreadNotifications()->count() }}</span>
      @endif
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="width: 25rem; height: 30rem; position: absolute; top: 2rem; left: -16rem;">
        ...
        <div class="card-body p-0 pt-2" style="position: absolute; top: 2.8rem; height:82%; width: 100%; overflow: scroll">
          ...
          <a class="dropdown-item d-flex flex-column announcement py-2" 
              ...
              data-title="{{ $notification->data['announcement']['announcement_title'] }}"
              data-employees="{{ $notification->data['announcement']['affected_employees'] }}"
              data-datefrom="{{ $notification->data['announcement']['date_from'] }}"
              data-timefrom="{{ $notification->data['announcement']['time_from'] }}"
              data-dateto="{{ $notification->data['announcement']['date_to'] }}"
              data-timeto="{{ $notification->data['announcement']['time_to'] }}"
              data-description="{{ $notification->data['announcement']['announcement_description'] }}"
              data-link="{{ $notification->data['announcement']['link'] }}"
              data-attachment="{{ $notification->data['announcement']['attachment'] }}"
              data-toggle="modal" data-target="#showAnnouncement">
              <h5 class="card-title" style="color: black">{{ $notification->data['announcement']['announcement_title'] }}</h5>
              <p class="card-text" style="color: black">{{ Carbon\Carbon::parse($notification->data['announcement']['date_from'])->format('M d') }}</p>
          </a>
          @empty
    ...
    <ul class="nav-item has-dropdown is-right is-hoverable m-0 is-desktop-up">
        <li onclick="this.closest('.has-dropdown').classList.toggle('is-active');" class="nav-item" style="height: 2rem; list-style: none;"><a class="has-arrow nav-link has-text-black" href="#" style="color: black;">{{ Auth::user()->fname }}</a></li>
