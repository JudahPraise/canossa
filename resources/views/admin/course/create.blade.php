  ...
  <form class="needs-validation mt-2" action="{{ route('course.store', $course->id) }}" method="post" novalidate>
    ...
    <div class="form-group inputs_div">
      <div class="form-row d-flex align-items-center">
        ...
        <div class="col-md-3 mb-3">
          <label for="validationCustom05">Teacher</label>
          <select class="custom-select" name="teacher_id[]" id="validationCustom04" required>
            <option selected disabled value=""></option>
            @forelse ($teachers as $teacher)
              <option value="{{ $teacher->id }}">{{ $teacher->lname.','.' '.$teacher->fname.' '.$teacher->mname.'.' }}</option>
              @empty
              <p>empty</p>
              @endforelse
            </select>
...
<script>
  ...
  $(document).ready(function(){
    $(this).on("click", ".addMore", function(){
      var html = '<div class="form-row d-flex align-items-center"><div class="col-md-3 mb-3"><label for="validationCustom03">Subject Code</label><input type="text" name="code[]" class="form-control" id="validationCustom03" required><div class="invalid-feedback">Please provide a valid city.</div></div> <div class="col-md-2 mb-3"><label for="validationCustom03">Units</label><input type="number" name="units[]" class="form-control" id="validationCustom03" required><div class="invalid-feedback">Please provide a valid city.</div></div><div class="col-md-3 mb-3"><label for="validationCustom05">Course Title</label><input type="text" name="description[]" class="form-control" id="validationCustom05" required><div class="invalid-feedback">Please provide a valid zip.</div></div><div class="col-md-3 mb-3"><label for="validationCustom05">Teacher</label><select class="custom-select" name="teacher_id[]" id="validationCustom04" required><option selected disabled value=""></option>@forelse ($teachers as $teacher)<option value="{{ $teacher->id }}">{{ $teacher->lname.','.' '.$teacher->fname.' '.$teacher->mname.'.' }}</option>@empty<p>empty</p>@endforelse</select></div><i class="fas fa-plus-square mr-2 mt-3 text-success addMore" style="font-size: 2rem"></i><i class="fas fa-minus-square mt-3 text-danger remove" style="font-size: 2rem"></i></div>'
