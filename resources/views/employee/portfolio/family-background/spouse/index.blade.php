    ...
    <div class="row pt-5 mb-3">
        ...
        <div class="col-6 d-flex justify-content-end align-items-center" style="height: 4rem;">
            ...
            @if (url()->current() != route('spouse.create'))
                <a href="{{ route('spouse.edit', Auth::user()->id) }}" class="{{ !empty(auth()->user()->family->spouse) ? 'neu-effect' : 'neu-effect-inset btn disabled' }} d-flex justify-content-center align-items-center text-decoration-none py-2 px-2" style="display:inline-block; "><i class="fas fa-edit" style="font-size: 1.6rem"></i></a>
            @endif
        </div>
    </div>
