    ...
    <div class="row mb-3">
        ...
        <div class="col-6 d-flex justify-content-end align-items-center" style="height: 4rem;">
            ...
            @if (url()->current() != route('father.create'))
                <a href="{{ route('father.edit', Auth::user()->id) }}" class="{{ !empty(auth()->user()->family->father) ? 'neu-effect' : 'neu-effect-inset btn disabled' }} d-flex justify-content-center align-items-center text-decoration-none py-2 px-2" style="display:inline-block; "><i class="fas fa-edit text-primary" style="font-size: 1.6rem"></i></a>
            @endif
        </div>
    </div>
