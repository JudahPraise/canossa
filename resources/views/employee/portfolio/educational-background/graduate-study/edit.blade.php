...
<div class="container-fluid" id="inputs">
  <form action="{{ route('grad.update', auth()->user()->education->id) }}" method="POST">
    @method('PUT')
      @csrf
      <div class="form-group inputs_div">
        @foreach ($grad as $data)
          <div class="form-group">
            <div class="form-row">
              <div class="col-md-6 mb-3">
                <label for="name_of_school">Name of school</label>
                <input type="text" class="form-control" name="name_of_school[]" value="{{ $data->name_of_school }}" id="name_of_school" required>
              </div>
              <div class="col-md-6 mb-3">
                <label for="validationDefault04">Degree</label>
                <select class="custom-select" name="degree[]" id="validationDefault04" required>
                  <option selected disabled value="">Choose...</option>
                  <option {{ $data->degree == 'Masters' ? 'selected' : '' }}>Masters</option>
                  <option {{ $data->degree == 'Doctorate' ? 'selected' : '' }}>Doctorate</option>
                </select>  
              </div>
            </div>
            <div class="form-row">
              ...
              <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control" name="level_units_earned[]" value="{{ $data->level_units_earned }}" id="level_units_earned" >  
              </div>
            </div>
            <div class="form-row">
              <div class="col-md-6 mb-3">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="sy_graduated[]" value="{{ $data->sy_graduated }}" id="sy_graduated" >
                </div>
              </div>
              <div class="col-md-6 mb-3">
                <label for="academic_reward">Academic reward</label>
                <input type="text" class="form-control" name="academic_reward[]" value="{{ $data->academic_reward }}" id="academic_reward" >
              </div>
            </div>
