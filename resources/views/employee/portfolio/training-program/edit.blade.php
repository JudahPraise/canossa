...
<div class="container-fluid">
    <form id="form" action="{{ route('training.update', $training->id) }}" method="POST"  enctype="multipart/form-data">
        @method('PUT')
        @csrf
            ...
            <div class="form-group">
                <div class="form-row">
                    <div class="col-md-6 mb-3">
                      ...
                      <input type="date" class="form-control" name="training_date" value="{{ $training->training_date }}" id="training_date" required>
                    </div>
                    <div class="col-md-6 mb-3">
                      <label for="training_title">Training Title</label>
                      <input type="text" class="form-control" name="training_title" value="{{ $training->training_title }}" id="training_title" >  
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="col-md-6 mb-3">
                      <label for="training_sponsor">Training sponsor</label>
                      <input type="text" class="form-control" name="training_sponsor" value="{{ $training->training_sponsor }}" id="training_sponsor" >  
                    </div>
                    <div class="col-md-6 mb-3">
                      <label for="sy_graduated">Training certificate</label>
                      <div class="custom-file">
                          <input type="file" class="custom-file-input" name="training_certificate" value="{{ $training->training_certificate }}" id="customFile">
                          <label class="custom-file-label" for="customFile">{{ $training->training_certificate }}</label>
                      </div>
                    </div>
                </div>
